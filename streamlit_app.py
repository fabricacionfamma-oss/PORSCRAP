...
# CREACIÓN DE LISTA BLANCA ESTRICTA SEGÚN ÁREA SELECCIONADA
if not df_sql_fil.empty:
    lista_blanca_sql = set(df_sql_fil['Código'].str.strip().str.upper().unique())
else:
...
if not df_gs_fil.empty:
    if len(lista_blanca_sql) > 0:
        ...
        # Filtro estricto: Descartamos todo lo que no esté detallado en el SQL para el área seleccionada
